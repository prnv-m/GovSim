    ...
    def _generate_recommendations(
        ...
        for vuln_type, vuln_issues in issue_types.items():
            if vuln_type == 'CODE_INJECTION':
                recommendations.append("⚠️ Remove eval() and exec() calls. Use safer alternatives like ast.literal_eval().")
            elif vuln_type == 'COMMAND_INJECTION':
                recommendations.append("⚠️ Avoid os.system() and subprocess with shell=True. Use subprocess with arguments list.")
            elif vuln_type == 'SQL_INJECTION':
                recommendations.append("⚠️ Use parameterized queries instead of string formatting for SQL.")
            elif vuln_type == 'OBFUSCATION':
                recommendations.append("⚠️ Remove unnecessary encoding/obfuscation. Code should be transparent.")
            elif vuln_type == 'SUSPICIOUS_IMPORT':
                recommendations.append("ℹ️ Verify that imported modules are necessary for this feature.")
